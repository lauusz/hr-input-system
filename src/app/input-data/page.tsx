...
export default function InputDataPage() {
  ...
  const uploadKK = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (res.ok) {
        setKkHeader({ noKK: json.data?.noKK || '' });
        setShowKkForm(true);
      } else {
            ...
            {showKtpForm && (
              <div className="animate-slide-up bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                <h3 className="text-lg font-bold text-gray-700 mb-4 border-b pb-2 flex items-center">
                  📝 Data KTP <span className="text-xs font-normal ml-2 text-gray-500">(Silakan koreksi jika ada yang salah)</span>
                </h3>
